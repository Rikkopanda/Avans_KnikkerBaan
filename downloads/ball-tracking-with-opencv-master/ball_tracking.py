...
# Store detected points as plain rows to avoid a hard pandas dependency.
Data_Points = []
...
while True:
	...
	if len(cnts) > 0:
		...
		if (radius < 300) & (radius > 10 ) : 
			...
			Data_Points.append([x, y, current_time])
...
if Data_Points:
	...
	theta = 2 * np.arctan(data[:, 1] * 0.0000762 / h)
	theta = theta - theta0

	# Creating the 'Theta' vs 'Time' plot.
	plt.plot(theta, data[:, 2])
	plt.xlabel('Theta')
	plt.ylabel('Time')

	# Export the data points as CSV and plot.
	with open('Data_Set.csv', 'w', newline='') as csv_file:
		writer = csv.writer(csv_file)
		writer.writerow(['x', 'y', 'time', 'theta'])
		for row, theta_value in zip(data, theta):
			writer.writerow([row[0], row[1], row[2], theta_value])
